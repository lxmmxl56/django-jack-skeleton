...
def index(request):
    ...
    error_alert = {'tag': 'danger', 'message': _('ERROR: This is just a test.')}
    info_alert = {'tag': 'info', 'message': _('Hiya Buddy 🌭')}
...
def restricted_media(request, filename=None, download=False):
    try:
        ...
        content_type, _ = MimeTypes().guess_type(os.path.basename(filename))
        if not content_type:
            log.debug("No content-type found for file: %s" % os.path.basename(filename))
        ...
        try:
            ...
            file_name = ReportFile.objects.get(
