...
class LoginView(SuccessURLAllowedHostsMixin, IdempotentSessionWizardView):
    ...
    that device is asked to generate a token (send email) and the
    ...
    def get_device(self, step=None):
        ...
        if not self.device_cache:
            ...
            if challenge_device_id:
                for device in backup_emails(self.get_user()):
    ...
    def render(self, form=None, **kwargs):
        """
        If the user selected a device, ask the device to generate a challenge sending an email.
        """
...
class SetupView(IdempotentSessionWizardView):
    ...
    condition_dict = {
        ...
        'validation': lambda self: self.get_method() == 'email',
    }
...
class EmailSetupView(IdempotentSessionWizardView):
    ...
    def get_context_data(self, form, **kwargs):
        ...
        kwargs.setdefault('cancel_url', resolve_url(settings.TWO_FACTOR_CANCEL_URL))
